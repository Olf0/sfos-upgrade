...
Not running it will result in the huge upgrade log files (containing many duplicate entries) and may result (as any SailfishOS upgrade at the command line without cleaning up afterwards) in RPMs failing to install ("unmet dependency" / "Fatal error: nothing provides X needed by Y" errors) plus annoying notifications from the store-client that an upgrade to the installed version is available.
